...
export default function RecibosPage() {
    ...
    const cargar = async () => { setCargando(true); setRecibos(await recibosStore.getAll()); setFacturas(await facturasStore.getAll()); setClientes(await clientesStore.getAll()); setCargando(false); };
    useEffect(() => { cargar(); }, []);
    ...
    const crearRecibo = async () => {
        ...
        if (!factura) return;
        const recibo = await recibosStore.create({ fecha: new Date().toLocaleDateString('es-ES'), factura_id: form.facturaId, cliente_id: factura.cliente_id, concepto: form.concepto || `Pago factura ${factura.numero || ''}`, importe: form.importe, firma_url: firmaTemp || undefined });
        const cliente = getCliente(factura.cliente_id);
        const doc = await generarPDFRecibo({ cliente: cliente?.nombre || "Cliente", concepto: form.concepto, importe: Number(factura.total), entrega: form.importe, restante: Number(factura.total) - form.importe, firma: firmaTemp || undefined });
        doc.save(`recibo-${recibo?.numero || 'nuevo'}.pdf`);
        await cargar(); setModalOpen(false); setFirmaTemp(null); setForm({ facturaId: "", concepto: "", importe: 0 });
    };
                ...
                <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-md z-[300] flex items-center justify-center p-6">
                    <div className="bg-white w-full max-w-lg rounded-[40px] shadow-2xl overflow-hidden">
                        ...
                        <div className="p-8 space-y-6">
                            <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Factura *</label>
                                <select value={form.facturaId} onChange={e => setForm({ ...form, facturaId: e.target.value })} className="premium-input"><option value="">-- Seleccionar factura --</option>{facturas.map(f => { const cl = getCliente(f.cliente_id); return <option key={f.id} value={f.id}>{f.numero} — {cl?.nombre || "—"} — {Number(f.total).toFixed(2)} €</option>; })}</select>
                                {facturas.length === 0 && <p className="text-[10px] font-bold text-amber-500 mt-2 uppercase tracking-widest">⚠ No hay facturas.</p>}</div>
                        ...
                        <div className="p-8 flex gap-4 bg-white border-t border-slate-50">
                            ...
                            <button onClick={crearRecibo} className="flex-1 bg-emerald-600 text-white px-8 py-5 rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-xl shadow-emerald-100 flex items-center justify-center gap-3 hover:bg-emerald-700 active:scale-95 border-none"><Save size={16} /> Crear y Descargar PDF</button>
                        </div>
                    </div>
                </div>
            )}
